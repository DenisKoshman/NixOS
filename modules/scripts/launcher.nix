...
let
  ...
  wallpaperThumbs = pkgs.runCommand "wallpaper-thumbnails" {
  ...
  '';
in pkgs.writeShellScriptBin "launcher" ''
  ...
  wallpaper)
    ...
    awww img "$WALLPAPER_DIR/$CHOICE" --transition-type random --transition-duration 1 --transition-fps 144
